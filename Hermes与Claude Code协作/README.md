...
- **134 条 Pitfalls**：完整反面案例库，按场景分类，含 TOC 导航
...
将 `SKILL.md` 加载到 Hermes 的上下文中。`references/` 下 29 个参考文档按需引用，涵盖会话管理、错误恢复、监控辩论、SSH 诊断、ACP 研究、文件传输等场景。
...
| 文件/目录 | 说明 |
|-----------|------|
| `SKILL.md` | Hermes 端操作流程总控（508 行，含 Print Mode + tmux 模式） |
...
| `monitoring-debate.md` | 监控与辩论协议（capture-pane 分析 + R1/R2/R3） |
