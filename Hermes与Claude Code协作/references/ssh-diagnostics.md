...
```
Hermes（云端）── SSH ──→ Windows（<Windows_用户名>@<Windows_Tailscale_IP>:<SSH_端口>）
                     ↑
                 ~/.ssh/config → local-win
```

> **SSH 配置：** `Host local-win` / `HostName <Windows_Tailscale_IP>`（Tailscale IP）/ `Port <SSH_端口>` / `User <Windows_用户名>`
...
SSH 连接失败
    ...
    │       netstat -an | findstr :<SSH_端口>          # 是否 LISTENING
    │    → 修复：Start-Service sshd
    │
    └── "Connection timed out" / Operation timed out
         │
         ├── Step 1: 区分端口状态
         │    nc -zv -w10 <Windows_Tailscale_IP> <SSH_端口>
         ...
         │    tailscale status | grep 100.107.xxx
         │    tailscale ping -c 3 <Windows_Tailscale_IP>
         │    │
         ...
         └── Step 3: 检查 Windows 防火墙
              ...
              → 如果无入站允许规则，添加：
                 New-NetFirewallRule -DisplayName "SSH <SSH_端口>" -Direction Inbound -Port <SSH_端口> -Protocol TCP -Action Allow
...
```bash
# 用户本地 VSCode 终端执行
ssh ubuntu@<云端_Tailscale_IP> -p <SSH_端口>    # 云端 Tailscale IP，端口 <SSH_端口>
...
```bash
# 云端（Hermes）
tailscale status | grep 100.107.xxx         # 连接状态和模式
tailscale ping <Windows_Tailscale_IP>              # Tailscale 层 ping
nc -zv -w10 <Windows_Tailscale_IP> <SSH_端口>            # 端口级别测试
ssh -v -o ConnectTimeout=10 local-win "echo OK"  # 详细 SSH 调试日志

# Windows 端（用户手动执行）
Get-Service sshd                             # SSH 服务状态
netstat -an | findstr :<SSH_端口>                  # 端口监听状态
...
| 3 | 稳定连接 | ✅ | — |
| 4 | SCP 超时 + SSH 不通 | 云端 Tailscale stale（长时间运行后 DERP 连接池耗尽） | `sudo systemctl restart tailscaled` 后立即恢复 |
