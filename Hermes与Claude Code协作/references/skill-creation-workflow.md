...
- [核心原则](#核心原则)
  - [文件复制规范](#文件复制规范)
  - [Permission 处理技巧](#permission-处理技巧)
- [标准项目结构](#标准项目结构)
- [Hermes SKILL.md frontmatter 规范](#hermes-skillmd-frontmatter-规范)
- [SKILL.md 架构原则（2026-06-03 讨论确认）](#skillmd-架构原则（2026-06-03-讨论确认）)
  - [两种架构模式](#两种架构模式)
  - [Progressive Disclosure 模式详解（2026-06-08 验证）](#progressive-disclosure-模式详解（2026-06-08-验证）)
  - [聚合 SKILL.md — 纯路由表，不调脚本](#聚合-skillmd-纯路由表，不调脚本)
  - [子 SKILL.md — 含脚本命令，路径相对于自身目录](#子-skillmd-含脚本命令，路径相对于自身目录)
  - [执行链路（完整流程）](#执行链路（完整流程）)
- [`/skill-creator` 使用流程](#skill-creator-使用流程)
  - [启动](#启动)
  - [分批策略（以 health-management 为例）](#分批策略（以-health-management-为例）)
  - [interview 表单交互](#interview-表单交互)
  - [`/skill-creator` 自动完成的工作](#skill-creator-自动完成的工作)
- [CC 自动触发的常见坑](#cc-自动触发的常见坑)
- [逐批自我审核（⚠️ 关键步骤）](#逐批自我审核（-关键步骤）)
  - [审核清单](#审核清单)
  - [自审输出格式](#自审输出格式)
- [ClawHub 包适配（不轻信 SKILL.md 描述）](#clawhub-包适配（不轻信-skillmd-描述）)
  - [验证方法](#验证方法)
  - [踩坑案例（2026-06-03）](#踩坑案例（2026-06-03）)
  - [注意事项](#注意事项)
  - [适配步骤](#适配步骤)
- [路由路径强制检查](#路由路径强制检查)
- [偏差讨论协议（非传话模式）](#偏差讨论协议（非传话模式）)
  - [讨论前置规则](#讨论前置规则)
  - [Point-by-point 讨论模式](#point-by-point-讨论模式)
  - [共识→执行转换](#共识执行转换)
  - [残留引用清理清单](#残留引用清理清单)
- [执行中需求变更处理](#执行中需求变更处理)
  - [变更影响计算](#变更影响计算)
