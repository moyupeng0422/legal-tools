...
flowchart LR
    ...
    Windows[CC (Windows)] -->|curl http://<云端_Tailscale_IP>:18888/file| HTTP
...
```bash
tmux send-keys -t claude-session 'curl -s http://<云端_Tailscale_IP>:18888/my-task-file.md -o C:\\Users\\<Windows_用户名>\\my-task-file.md' Enter
```
...
|--------|-----------|--------|
| `scp -P <SSH_端口> file <Windows_用户名>@<Windows_Tailscale_IP>:C:/Users/...` | Cloud→Windows | ❌ Times out (60s+) |
| `cat file \| ssh -p <SSH_端口> <Windows_用户名>@<Windows_Tailscale_IP> "powershell ..."` | Cloud→Windows | ❌ Times out (90s+) |
...
- The cloud's Tailscale IP (`<云端_Tailscale_IP>` for this setup) must be reachable from Windows
...
- SCP/SCP-r times out but SSH works (`ssh -p <SSH_端口> <Windows_用户名>@<Windows_Tailscale_IP> "echo OK"` succeeds)
