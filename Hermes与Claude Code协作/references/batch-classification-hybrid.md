...
- [问题背景](#问题背景)
- [三阶段方案](#三阶段方案)
  - [Phase 1 — Python 规则引擎（脚本执行，不占 CC context）](#phase-1-python-规则引擎（脚本执行，不占-cc-context）)
  - [Phase 2 — CC 逐条审核（只处理不确定的 15-20%）](#phase-2-cc-逐条审核（只处理不确定的-15-20）)
  - [Phase 3 — 数据完整性验证](#phase-3-数据完整性验证)
- [CC Context 溢出后的续接](#cc-context-溢出后的续接)
- [实战数据（2026-06-11，微信群「法律AI加油站」噪音筛选）](#实战数据（2026-06-11，微信群「法律ai加油站」噪音筛选）)
- [技术细节](#技术细节)
  - [Phase 4 — 数据合并与最终验证](#phase-4-数据合并与最终验证)
  - [Phase 4b — 后分类内容合并（筛选记录→完整内容清单）](#phase-4b-后分类内容合并（筛选记录完整内容清单）)
- [K原 | 2026-05-27 07:03 | wxid_xxx](#k原-2026-05-27-0703-wxid_xxx)
  - [Phase 5 — Python 统计分析脚本（替代 CC 手动逐条读取）](#phase-5-python-统计分析脚本（替代-cc-手动逐条读取）)
