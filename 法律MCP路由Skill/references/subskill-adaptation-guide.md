...
子skill 的调用不止法律 MCP（企业数据/网页/文件处理等），处理规则：

| cost_level | 例子 | 规则 |
|---|---|---|
| **free** | 网页阅读、搜索、文件读写 | 子skill **自主调用**，步骤汇报列明即可 |
| **paid** | 企查查企业数据、天眼查、智慧芽等计费 MCP | **主agent 审核方案时执行三段判断**（见下）；企查查企业数据另须加载 `qcc-enterprise-bridge.md` |
...
3. **无 → 本skill 通用兜底**：子skill 首次调用前向主agent 报备（工具+预计次数），主agent 按单类**一次性确认**，确认后同类调用不再逐次打断；实际调用超确认次数 50% 再报备。企查查企业数据按内置桥规则执行主体锚定、risk scan 后下钻、当前/历史互斥，成本记 `null` 待平台账单核对
