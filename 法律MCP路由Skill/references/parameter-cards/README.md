...
> **框架化定位（2026-08-21）**：本索引与各卡 = **已知7 MCP 知识库**。①卡内工具是否可用以 `../../data/user-profile.json` 的 mcp_inventory 为准（未启用/不在线自动跳过）②知识库外的 MCP 走通用模式——读该工具的 tool description 定参数，不猜格式。③profile.path_order_overrides 非空时，卡内"按升级顺序"被该功能覆盖序取代（仅调序，红线不豁免，见 onboarding-guide 附录）。**功能覆盖度兜底**（见升级表 4.0）：某功能在当前 profile 中无可用工具时，标注"本 profile 未覆盖功能N"并继续，不得硬凑。
...
| 功能 | 卡片 | 核心工具（按升级顺序） |
|---|---|---|
| 功能1 法条精准查询 | `f1-law-exact.md` | 法信law_search/detail · 元典ft_detail · 北大法宝get_law_item_content · 法研iterative · 威科search_law_article · FLK hit_display（仅"法规名+关键词"变体） |
| 功能2 语义找法条 | `f2-semantic-law.md` | FLK拆词 · 法信拆词 · 元典law_vector_search · 北大法宝search_article · 威科search_law |
| 功能3 精准找案例 | `f3-case-exact.md` | RMFYALK case_ref/key_content · 法信case检索 · 元典case_details · 北大法宝get_case_list |
| 功能4 语义找案例 | `f4-semantic-case.md` | 法信拆词 · 元典case_vector_search · 北大法宝search_case · 威科search_case |
| 功能5 类案检索 | `f5-case-search.md` | 法信case_search(20+维度) · 北大法宝get_case_list · 元典qwal/ptal |
| 功能6 权威案例 | `f6-authoritative.md` | RMFYALK search · 元典qwal · 北大法宝get_case_list(caseGrade) |
| 功能7 时效核验/修订历史 | `f7-timeline.md` | 法信沿革 · FLK历史版本 · 法研status · 北大法宝get_law_list |
| 功能8 引用校验/防幻觉 | `f8-citation.md` | 元典hall_detect(API直调) · 回归功能1/3一般检索 |
...
3. **法信/FLK/法研无语义引擎**——语义检索必须用北大法宝/元典/威科；FLK/法信只做LLM拆词
...
9. **调用包裹格式两种口径（2026-08-27 立，retest-F1-20260827 建议7）**——**法信/FLK/RMFYALK 侧工具参数须包一层 `{"params": {...}}`**；**元典/北大法宝/法研/威科侧参数平铺直传**（不包 params）。各功能卡头部有本卡适用的口径提示行；f8 无 MCP 直调不适用。子agent 加载 schema 后先对位包裹格式再填参，可减少 schema 研读往返
