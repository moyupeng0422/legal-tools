...
> ⚠️ **调用包裹格式（本卡涉及：法信·FLK=包裹 / 企查查·元典·北大法宝·法研=平铺）**：法信/FLK 侧工具参数须包一层 `{"params": {...}}`；企查查/元典/北大法宝/法研侧参数**平铺直传**，不包 params。企查查完整参数见 `qcc-legal.md`。

> 用户2026-08-04区分两种情况：
> - **仅时效核验**（确认法条现行有效/失效）——除RMFYALK外全部可用；企查查支持 `effectStatus` 与 `referDate`
...
> 企查查低成本补充：免费源未命中/需第二来源时，用 `get_legal_regulation_search(keyword/effectStatus/日期)` 或 `get_legal_article_detail(regulationName, articleNo, referDate)`（1分）。它能核对目标时点内容，但不提供完整历次修订列表，完整参数见 `qcc-legal.md`。
