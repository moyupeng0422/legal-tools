...
单次成本 = credit-dictionary.json → mcp[tool].tools[tool]
         （未收录工具按 default_cost；免费层=0；
           profile cost_known:false 或动态套餐成本 MCP = null，不参与积分统计）
```
...
| 企查查 get_legal_article_detail | 1分（查字典） |
| 企查查 get_legal_article_semantic_search | 3分（查字典） |
| 企查查 get_judicial_case_search | 3分（查字典） |
| 企查查企业数据工具 | null（价格随部署/套餐变化，按预计调用次数确认） |
...
```
【积分池】recurring（北大法宝/元典/企查查法律数据）+ free_trial 消耗
...
【动态成本池】企查查企业数据
   cost = null，不与积分池相加；累计实际调用次数
   调用前按类别确认预计次数，超确认量50%再次报备，任务后以平台账单核对
```
...
> - **北大法宝/元典/企查查法律数据**：recurring 积分或套餐额度——**不做单次确认，做任务预算控制**
...
| **企查查法律数据** | **任务预算控制** | 法规工具1/3分、案例工具3分；按 profile `budget_per_task` 累计，超预算停下询问 |
| **企查查企业数据** | **预计次数确认** | 成本动态，首次按类别一次性确认预计次数；超确认量50%再报备；cost=null |
...
③ 按 1.3 分策略核对：北大法宝/元典/企查查法律数据查任务预算；企查查企业数据查预计次数；威科逐次确认
④ 批准后进入执行（□3预算预检 → □4收费确认）
```
...
| `cost` | number\|null | ✅ | 本次消耗（积分/次/0；威科按"次"；**null=未知或动态成本**——含 profile cost_known:false 与企查查企业桥，不参与积分对账） | `25` |
...
- **预算上限来源**：子agent在□2方案审核时从主agent获得批准方案中的预算上限（北大法宝≤500/元典≤50/企查查法律按profile预算/企查查企业按预计次数/威科逐次），执行中自行累计对照
